...
An automated, containerized batch pipeline that integrates three public data sources with different APIs, formats, cadences, and country-coding schemes into a reconciled, tested data warehouse.
...
Labor market, economic, and climate-vulnerability data come from three different institutions, in three different formats, on three different cadences, and under three different country-identification schemes.

This pipeline ingests each source, lands it immutably, cleans and conforms it, reconciles every source to a single country key, and builds a wide, tested country-year mart that a downstream analyst or model can consume directly.

Variables are reconciled and co-located. Interpretation is left to whoever queries the mart.
...
Reconciling three country-identification schemes (ISO-2 / ISO-3 / names) into one conformed key is the main integration task, handled once in `dim_countries` so every downstream model joins cleanly.
...
```
        ...
        Staging (one model per source: clean, type, reshape)
             ...
             dim_countries  (ISO reconciliation, conformed key)
                                 │
        fct_labor_conditions  ← reconciled, tested
...
1. **`fct_labor_conditions`**: a wide, tested country-year mart integrating labor and economic indicators, with climate-vulnerability context from the reconciled sources, documented column by column.
...
A broken, drifted, or stale source fails the build instead of corrupting the mart.
...
ND-GAIN downloads automatically when its DAG runs.
